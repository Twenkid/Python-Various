...
# However it didn't received the keys as it was out-of-the box (on my Python 3.8)
# I fixed it by checking the pressed keys and adding them to a set one by one, see "run"
# Extended with j - jump (allows flying) etc.
...
D = enum.Enum('D', 'n e s w j')                   # Direction #j - jump
...
def run(screen, images, mario, tiles):
    ...
    while all(event.type != pg.QUIT for event in pg.event.get()):
        keys = {pg.K_UP: D.n, pg.K_RIGHT: D.e, pg.K_DOWN: D.s, pg.K_LEFT: D.w, pg.K_j: D.j}        
        ...
        if keyPressed[pg.K_j]: pressed.add(D.j) #'}  #{'s'} #{
...
def update_speed(mario, tiles, pressed):
    x, y = mario.spd
    #print(D.e, pressed)
    x += 3 * ((D.e in pressed) - (D.w in pressed)) #+2
    print(x)
    x -= x // abs(x) if x else 0
    y += 1 if D.s not in get_boundaries(mario.rect, tiles) else (D.n in pressed) * - 25 #-10 jump -14 #+! ...
    y -= 10*(D.j in pressed)
